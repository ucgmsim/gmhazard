    ...
    for mode in modes:
        ...
        for i, nhyp in enumerate(nhyps):
            ...
            if mode == "std":
                ...
                colour_map.reverse()
                c = plt.contourf(x, y, values, levels=bounds, colors=colour_map, extend='both')
        ...
        if mode == "mean bias":
            fig.savefig(
                f"/home/joel/local/directivity/latin/mc/depth_fix/spatial/{fault_name}_mean_bias.png"
            )
        else:
            fig.savefig(
                f"/home/joel/local/directivity/latin/mc/depth_fix/spatial/{fault_name}_{mode}.png"
            )
        plt.close()
