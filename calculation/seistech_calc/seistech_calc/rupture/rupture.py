...
RUPTURE_DF_COLUMNS = ["rupture_name", "annual_rec_prob", "magnitude"]
...
def rupture_df_from_erf(
    ...
    if erf_file_type is const.ERFFileType.flt_nhm:
        ...
        rupture_dict = {
            f"{info.name}_{erf_name}": [info.name, 1 / info.recur_int_median, info.mw]
            for key, info in nhm_infos.items()
            if info.recur_int_median > 0
        }

        return pd.DataFrame.from_dict(
            rupture_dict, orient="index", columns=RUPTURE_DF_COLUMNS
        ).sort_index()

    elif erf_file_type is const.ERFFileType.ds_erf:
        df = pd.read_csv(erf_ffp)
        df.columns = RUPTURE_DF_COLUMNS
